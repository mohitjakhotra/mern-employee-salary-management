...
export const Login = async (req, res) => {
  try {
    ...
    const hash = pegawai.password || '';
    if (typeof hash !== 'string' || !hash.startsWith('$')) {
      return res.status(400).json({ msg: "Password user tidak valid. Silakan reset password." });
    }

    const match = await argon2.verify(hash, req.body.password);
